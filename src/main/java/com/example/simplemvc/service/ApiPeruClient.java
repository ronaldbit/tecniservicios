...
public ApiPeruClient(RestTemplate http,
  @Value("${apiperu.base-url}") String baseUrl,
  @Value("${apiperu.token}") String token) {
    this.http = http; this.baseUrl = baseUrl; this.token = token;
  }

  public StdOk<DniData> consultarDni(String dni){
    if (dni==null || !dni.matches("\\d{8}"))
      return StdOk.fail("DNI inválido (8 dígitos).");
    ...
    try{
      ...
      ResponseEntity<DniResponse> resp = http.exchange(
        baseUrl + "/dni", HttpMethod.POST, req, DniResponse.class);

      DniResponse body = resp.getBody();
      if (body==null) return StdOk.fail("Sin respuesta.");
      if (!body.success()) return StdOk.fail(body.message()!=null?body.message():"No encontrado.");
  ...
  public StdOk<RucData> consultarRuc(String ruc){
    if (ruc==null || !ruc.matches("\\d{11}"))
      return StdOk.fail("RUC inválido (11 dígitos).");
    ...
    try{
      ...
      ResponseEntity<RucResponse> resp = http.exchange(
        baseUrl + "/ruc", HttpMethod.POST, req, RucResponse.class);
