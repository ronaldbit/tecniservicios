...
public class DbUserDetailsService implements UserDetailsService {
  ...
  public DbUserDetailsService(JdbcTemplate jdbc) { this.jdbc = jdbc; }

  @Override
  public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
    try {
       
      var u = jdbc.queryForMap("""
        SELECT id, usuario AS username, password AS password_hash, activo
        FROM usuario
        WHERE usuario = ? OR email = ?
      """, username, username);

      boolean enabled = u.get("activo") == null || Integer.valueOf(String.valueOf(u.get("activo"))) == 1;
      ...
      return User.builder()
        ...
        .password((String) u.get("password_hash")) // BCrypt en DB
        .authorities(auths)
        .disabled(!enabled)
        .build();
